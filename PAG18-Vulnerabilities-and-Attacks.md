...
Una condición de carrera (Race Condition) es una vulnerabilidad que ocurre cuando dos o más procesos o hilos intentan acceder o modificar "el mismo recurso al mismo tiempo", y el resultado depende del orden en que se ejecutan.

En otras palabras, el atacante aprovecha un pequeño intervalo de tiempo para ejecutar una acción antes que el sistema, alterando el comportamiento esperado.

Vamos a repasar estos 2 conceptos antes de imaginar

Puntero: Un puntero es una variable que guarda la dirección de memoria donde se encuentra otro dato.
Desreferenciar (dereference): significa acceder al dato al que apunta ese puntero.

Ejemplo:

Puntero → Dirección 0x1000 → Variable = 50

Cuando el programa desreferencia el puntero, lee el valor 50.

¿Dónde aparece la vulnerabilidad?

Si dos hilos (o un atacante y el programa) acceden al mismo puntero al mismo tiempo, puede ocurrir esto:

- El programa comprueba que el puntero apunta al dato correcto.
- Antes de usarlo, otro hilo cambia ese puntero para que apunte a otra dirección de memoria.
- El programa termina utilizando un dato diferente al esperado.

"Es decir, se rompe el emparejamiento entre el puntero y aquello a lo que apuntaba".

Ejemplo sencillo:

Imagina que el puntero apunta a un archivo:

Puntero → archivo_seguro.txt

El programa verifica que es seguro.

Pero justo antes de abrirlo, un atacante cambia el puntero a:

Puntero → archivo_malicioso.txt

El programa cree que está abriendo el archivo seguro, pero en realidad abre el archivo malicioso.

Ahora sigamos con Condición de Carrera...

¿Cómo funciona la Condición de Carrera?

Imagina que un programa realiza estos pasos:

Comprueba si tienes permiso para hacer una acción.
Espera unos milisegundos.
Ejecuta la acción.

Durante ese pequeño intervalo, un atacante puede modificar algo para obtener un beneficio.

Ejemplo bancario (muy típico)
...
1. Verifica saldo
2. Autoriza transferencia
3. Descuenta dinero

Ahora el atacante envía 10 solicitudes exactamente al mismo tiempo.

Cada una ve:

Saldo = $100

Todas son aprobadas antes de que el saldo se actualice.
...
El atacante termina retirando mucho más dinero del disponible. Eso es una Race Condition.

Dirty COW (ejemplo real)

Dirty COW fue una vulnerabilidad muy famosa en Linux.

COW significa:

Copy-On-Write

Normalmente un archivo de solo lectura no puede modificarse.

Pero el atacante lograba:

Leer archivo

↓

Modificar memoria exactamente
en el momento correcto

↓

El sistema confundía la copia

↓

Permitía escribir donde solo podía leer
...
Todavía no hizo la transferencia.

2. TOU (Time Of Use)

Tiempo de uso.

Después de comprobar el saldo...

...el atacante modifica la situación.

Cuando llega el momento de usar esa información:

El saldo ya cambió.

Pero el sistema sigue usando la comprobación antigua.

TOCTOU

En realidad TOC y TOU forman parte de la misma vulnerabilidad, conocida como:

TOCTOU (Time Of Check To Time Of Use).

Es una de las Race Conditions más conocidas.

3. TOE (Time Of Evaluation)

Aquí el atacante modifica datos mientras el sistema está calculando o evaluando algo.
...
el atacante cambia el precio del producto.

El cálculo termina usando información modificada.
...
Cuando un proceso utiliza un recurso:
...
Un Mutex permite que solo un hilo ejecute una sección crítica del código a la vez.
...
Nunca trabajan simultáneamente sobre el mismo dato.

Ejemplo de SharePoint

Cuando editas un documento:
...
Eso evita Race Conditions.

Problema de usar Locks: Deadlock

A veces aparecen bloqueos.
...
Todo queda detenido hasta que alguien libere el recurso o intervenga el sistema. 

Para evitar el bloqueo, es importante que diseñes y pruebes adecuadamente cualquier bloqueo o mutex que utilices dentro de tu código para evitar una condición de carrera.

#### Resúmen (Tabla + Texto)

| Concepto                     | Descripción                                                                                          |
| ---------------------------- | ---------------------------------------------------------------------------------------------------- |
| **Race Condition**           | Dos procesos acceden al mismo recurso al mismo tiempo y el resultado depende del orden de ejecución. |
| **Dirty COW**                | Vulnerabilidad de Linux basada en una Race Condition que permitía escalar privilegios.               |
| **TOC (Time Of Check)**      | El sistema verifica un recurso.                                                                      |
| **TOU (Time Of Use)**        | El recurso cambia entre la verificación y su uso.                                                    |
| **TOCTOU**                   | La combinación de TOC y TOU; una de las Race Conditions más comunes.                                 |
| **TOE (Time Of Evaluation)** | El atacante modifica datos mientras el sistema los está evaluando.                                   |
| **Lock**                     | Bloquea un recurso para que solo un proceso pueda usarlo.                                            |
| **Mutex**                    | Garantiza que solo un hilo ejecute una sección crítica del código a la vez.                          |
| **Deadlock**                 | Dos procesos quedan bloqueados esperando recursos mutuamente.                                        |

Por lo tanto, recuerde, una condición de carrera es una vulnerabilidad del software que se produce cuando el resultado resultante de los procesos de ejecución depende directamente del orden y
el tiempo de ciertos eventos, y esos eventos no se ejecutan en el orden y el tiempo previsto por el desarrollador.

Hay tres tipos principales de condiciones de carrera que se pueden encontrar en el mundo real, incluyendo una vulnerabilidad de tiempo de comprobación, una vulnerabilidad de tiempo de uso y una
vulnerabilidad de tiempo de evaluación.

Todas estas condiciones de carrera pueden tener graves repercusiones en la seguridad y fiabilidad de sus aplicaciones informáticas.

Por último, es importante recordar que para evitar que una condición de carrera se utilice con éxito, es necesario mitigar estas vulnerabilidades mediante el uso de mutexes o bloqueos de recursos.

Idea clave para recordar

Race Condition = aprovechar el "timing" (momento exacto) para ganar una carrera contra el sistema.
TOCTOU = el recurso cambia entre que el sistema lo comprueba y cuando finalmente lo usa.

La principal defensa es sincronizar el acceso mediante locks o mutexes, evitando que varios hilos modifiquen el mismo recurso simultáneamente.
